```{r instalar e carregar pacotes}
  ...
  pacotes <- c("DALEX", "vip", "shapper", "tidyverse", "lubridate", "janitor", "dplyr", "xlsx", "stringr", "tidyr", "plotly", "keras", "fastDummies", "tensorflow", "equatiomatic", "kableExtra", "stargazer", "caret", "ROCR", "pROC")
  ...
  # Tensorflow - mudei o ambiente do python via preferencias globais do RStudio - no caso coloquei num ambiente virtual   
```
...
```{r arquivos IDESP e um wrangling básico}
  ...
  # Função para converter colunas com dados do tipo fator para numérico
  convert_factor_to_numeric <- function(df) {
    colunas <- grep("IDESP", names(df), value = TRUE) # Identificar colunas com "IDESP" no nome
      for (col in colunas) {
        if (is.factor(df[[col]]) || is.character(df[[col]])) {
          
          valor_str <- as.character(df[[col]]) # Converte o valor para string
          valor <- as.numeric(gsub(",", "", valor_str)) # Remove vírgulas
          casas_decimais <- nchar(sub(".*,", "", valor_str)) # Conta o número de casas decimais
          
          # Aplica a lógica condicional para divisão conforme o número de casas decimais
          df[[col]] <- ifelse(
            grepl(",", valor_str), # Verifica se contém vírgula
        ...
        # Tratamento da coluna CODESC
        if ("CODESC" %in% names(df)) { # Verifica se a coluna CODESC existe
          if (!is.integer(df$CODESC)) { # Se não for fator, converte
            df$CODESC <- as.integer(as.character(df$CODESC))
          }
        }
      }
      return(df)
  ...
  # Aplicar a função em todos os dataframes de interesse
  idesp17 <- convert_factor_to_numeric(idesp17)
  idesp18 <- convert_factor_to_numeric(idesp18)
  idesp19 <- convert_factor_to_numeric(idesp19)
  idesp21 <- convert_factor_to_numeric(idesp21)
  idesp22 <- convert_factor_to_numeric(idesp22)
  
  # janitor::compare_df_cols(idesp17, 
  #                          idesp18,
  #                          idesp19,
  #                          idesp21,
  #                          idesp22)
  ...
  # Código da versão 1.0.1 - Evita problemas quando os valores são inteiros ou tem apenas uma casa decimal
  # idesp17 <- idesp17 %>%
...
```{r arquivos M_IDESP e um wrangling básico}
  ...
  # Aplicar a função em todos os dataframes de interesse
  m_idesp17 <- convert_factor_to_numeric(m_idesp17)
  m_idesp18 <- convert_factor_to_numeric(m_idesp18)
  m_idesp19 <- convert_factor_to_numeric(m_idesp19)
  m_idesp20 <- convert_factor_to_numeric(m_idesp20)
  m_idesp21 <- convert_factor_to_numeric(m_idesp21)
  m_idesp22 <- convert_factor_to_numeric(m_idesp22)
  
  janitor::compare_df_cols(m_idesp17, 
                           m_idesp18,
                           m_idesp19,
                           m_idesp20,
                           m_idesp21,
...
Algumas das tabelas tem só o código administrativo ao invés do código da escola, então criei uma tabela com esses dois códigos para poder inserir o código da escola nas tabelas que não tiverem, isso fará que o merge entre tabelas seja facilitado futuramente
